...
def make_map_fn_difficulty(split, source=None):
        def process_fn(example, idx):
            if source is None:
                data_source = example.pop("source")
            else:
            ...
            question = example.pop("prompt")
            solution = example.pop("answer")
            difficulty_level = example.pop("difficulty")
...
if __name__ == '__main__':
    ...
    test_dataset = test_dataset.map(function=make_map_fn_difficulty("test", data_source), with_indices=True)
