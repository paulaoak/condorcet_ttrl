...
from verl.trainer.ppo.metric_utils import (
    ...
    process_validation_metrics_last,
)
...
class RayPPOTrainer:
    ...
    def _last_validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            # repeat test batch
            test_batch = test_batch.repeat(
                repeat_times=self.config.actor_rollout_ref.rollout.val_kwargs.n_last, interleave=True
            )
        ...
        for data_source, var2metric2val in data_src2var2metric2val.items():
            ...
            for var_name, metric2val in var2metric2val.items():
                ...
                for metric_name, metric_val in metric2val.items():
                    if (
                        (var_name == core_var)
                        and any(metric_name.startswith(pfx) for pfx in ["maj"])
                        and (f"@{n_max}" in metric_name)
                    ):
                        metric_sec = "val-core-final"
                    else:
                        metric_sec = "val-aux-final"
                    pfx = f"{metric_sec}/{data_source}/{var_name}/{metric_name}"
                    metric_dict[pfx] = metric_val
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                if is_last_step:
                    ...
                    pprint(f"Final validation metrics: {last_val_metrics_adaptive}")
                    progress_bar.close()
                    return
